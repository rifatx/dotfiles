...
PROMPT_COMMAND='PS1_GITCMD="$(b=$(git branch 2>/dev/null | grep '"'"'*'"'"' | colrm 1 2); [[ ${#b} -gt 0 ]] && cfu=$(("0$(git diff --shortstat | sed -r '"'"'s/[ ]*([0-9]+) file[s]* changed.*/\1/'"'"')")) && cfs=$(("0$(git diff --cached --shortstat | sed -r '"'"'s/[ ]*([0-9]+) file[s]* changed.*/\1/'"'"')"))  && echo -n "${b}/$((cfu+cfs)) ")";PS1_VENVCMD="$(v=${VIRTUAL_ENV##*/}; [[ ${#v} -gt 0 ]] && echo -n "(${v}) ")"'
